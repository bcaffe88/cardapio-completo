...
export async function getCommissionStats() {
  ...
  const totalRevenue = orderResults.reduce((sum, o) => sum + parseFloat(o.total || '0'), 0);
  ...
  for (const order of orderResults) {
    ...
    if (client) {
      ...
      totalCommissions += Math.floor(
        (orderTotal * commissionRate) / 10000
      );
    }
  }
...
export async function createRestaurantClient(data: {
  ...
  const result = await db.insert(restaurantClients).values({
    ...
    isActive: true,
  }).returning({ id: restaurantClients.id });

  return { id: result[0].id, tempPassword };
}
...
export async function createDeliveryPerson(data: {
  ...
  const result = await db.insert(deliveryDrivers).values({
    ...
    isActive: true,
    isAvailable: true,
  }).returning({ id: deliveryDrivers.id });
...
export async function updateRestaurantSettings(
  ...
  if (settings.deliveryFee !== undefined) updateData.deliveryFee = String(settings.deliveryFee);
  if (settings.minimumOrder !== undefined) updateData.minimumOrder = String(settings.minimumOrder);
